...
public sealed class Task4Drawer : MonoBehaviour
    // todo: extract shared code
{
    ...
    private void Update()
    {
        if (!TryDraw())
        {
            Graphics.CopyTexture(renderTextureSrc, renderTextureDst);
        }
    }
    ...
    private bool TryDraw()
    {
        ...
        Graphics.Blit(renderTextureSrc, renderTextureDst, drawMaterial);
